...
サブセクションでは、AI 特有ではない脅威、モデルポイズニング、モデル漏洩、安全でない出力処理、入力データの漏洩、拡張データへの攻撃、エージェントエスケープ、エージェントサンドボックス化をカバーします。
...
Input data (e.g., GenAI prompts) can be sensitive and can either leak through a failure or through an attack, such as a man-in-the-middle attack.  
