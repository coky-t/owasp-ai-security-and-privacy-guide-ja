...
- **Case-fold locale-independently, not with a locale-sensitive lowercase**: When normalizing text before matching it (e.g. against a blocklist), fold case with a locale-independent operation such as `str.casefold()` (Python) or ICU `toCaseFold`, and never a locale-sensitive lowercase. On a host configured for a Turkish locale, `"IGNORE".toLowerCase()` maps ASCII `I` (U+0049) to `ı` (U+0131, dotless i), so a check for `ignore` no longer matches; `casefold` avoids this and also handles cases a plain lowercase misses (e.g. `ß` → `ss`). Note that Unicode normalization alone does not fold case: `NFKC("İGNORE")` is still `"İGNORE"`, so normalization and case-folding are separate, both-required steps.
- **Collapse confusable characters and flag mixed scripts**: Unicode normalization does not remove homoglyph confusables. Cyrillic or Greek look-alikes such as `а` (U+0430) or `і` (U+0456) survive NFKC unchanged and can spell a blocklisted word that a later byte comparison then misses. Map input to its confusable skeleton (Unicode UTS #39, [Unicode Security Mechanisms](https://www.unicode.org/reports/tr39/)) before matching, and treat mixed-script runs within a single token as an anomaly signal, since legitimate words rarely mix scripts.
