...
このリソースの次の大きなページは、すべての AI セキュリティ脅威とそのコントロールに関する広範な詳細です。
...
### 1. リスクの特定 - 決定ツリー
組織に影響を及ぼす可能性のある潜在的なリスクを発見するには、該当する脅威の技術的およびビジネス的な評価が必要です。このドキュメントでは、AI 固有のリスク、つまり AI 固有の資産に対するリスクのみに焦点を当てています。以降のセクションでは種類ごとにリスクの影響について概説し、自分のケースに当てはまるリスクを特定します。
...
> ALWAYS DO:
  ...
  - You: consider the threat of [data poisoning](/go/datapoison/) in the development environment or by obtaining poisoned data from a third party, and consider the threat of attackers altering your model directly using [direct development-time model posoning](/go/devmodelpoison/).
  ...
  > QUESTION: Does your system insert (augment) data to the input of your model, like for example in RAG (Retrieval Augmented Generation), or by having _system prompts_ (standard instructions to the model that are automatically added to the input) ? If yes: 
...
|9| [リソース枯渇](2_threats_through_use.md#24-failure-or-malfunction-of-ai-specific-elements-through-use) はどの IT システムでも問題になる可能性がありますが、生成 AI モデルは一般的に実行コストが高いため、過負荷になると不要なコストが発生する可能性があります。 | ([OWASP for LLM 10: Unbounded consumption](https://genai.owasp.org/llmrisk/llm10/)) |
