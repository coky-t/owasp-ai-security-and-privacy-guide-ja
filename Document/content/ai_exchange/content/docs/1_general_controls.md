...
セキュア開発プログラム: セキュリティがソフトウェアに組み込まれていることを確保するためのプロセスを整備します。これらのプロセスの例: セキュア開発トレーニング、コードレビュー、セキュリティ要件、セキュアコーディングガイドライン、脅威モデリング (AI 固有の脅威を含む)、静的解析ツール、動的解析ツール、ペネトレーションテスト。

目的: ソフトウェア開発時にリスクを軽減するために適切な注意を払うことでセキュリティリスクを低減します。

AI 固有のサプライチェーンのリスクについて説明している [SUPPLYCHAINMANAGE](3_development_time_threats.md#SUPPLYCHAINMANAGE) については、このドキュメントの他の箇所を参照してください。
...
コンプライアンスのチェック: コンプライアンスマネジメント (セキュリティの側面を含む) において、AI 固有の法規制が考慮されていることを確認します。個人データが関連する場合や個人に関する意思決定に AI が適用される場合は、プライバシーの法規制も適用範囲となります。AI のプライバシーの側面については [OWASP AI ガイド](https://owasp.org/www-project-ai-security-and-privacy-guide/) を参照してください。
標準へのリンク:
