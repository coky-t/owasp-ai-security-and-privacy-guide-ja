...
説明: AI を制御するプログラムをインストールして実行します。組織として AI に責任を持ち、AI への取り組みの一覧表を作成し、リスク分析を行い、それらのリスクを管理します。
> カテゴリ: マネジメント
...
説明: 組織にセキュリティプログラム (_情報セキュリティマネジメントシステム_ ともよばれます) があり、AI ライフサイクル全体と AI 固有の側面を含むことを確認します。
> カテゴリ: マネジメント
...
#### #SECDEVPROGRAM
説明: データサイエンス開発活動を安全なソフトウェア開発プログラムの一部にします。
> カテゴリ: マネジメント
...
#### #CHECKCOMPLIANCE
(マネジメント)。コンプライアンスマネジメント (セキュリティの側面を含む) において、AI 固有の法規制が考慮されていることを確認します。個人データが関連する場合や個人に関する意思決定に AI が適用される場合は、プライバシーの法規制も適用範囲となります。AI のプライバシーの側面については [OWASP AI ガイド](https://owasp.org/www-project-ai-security-and-privacy-guide/) を参照してください。
標準へのリンク:
...
#### #SHORTRETAIN
説明: 短期保持: 必要がなくなったり、法的に (プライバシー法などにより) 要求された場合、データを削除または匿名化します。
> カテゴリ: 開発時および実行時
